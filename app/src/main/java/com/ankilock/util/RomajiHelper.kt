    ...
    fun kanaToRomaji(kana: String): String { 
        ...
        while (i < kana.length) { 
            if (kana[i] == 'っ' || kana[i] == 'ッ') { 
                if (i + 1 < kana.length) { 
                    val nextSub = if (i + 2 <= kana.length) kana.substring(i + 1, minOf(i + 3, kana.length)) else "" 
                    val nextRomaji = kanaMap[nextSub] ?: kanaMap[kana.substring(i + 1, i + 2)] ?: "" 
                    if (nextRomaji.isNotEmpty()) { 
                        sb.append(nextRomaji.first()) 
                    } 
                } 
                i++ 
                continue 
            ...
            val single = kana.substring(i, i + 1) 
            sb.append(kanaMap[single] ?: single) 
            i++ 
        } 
        return sb.toString() 
    } 
