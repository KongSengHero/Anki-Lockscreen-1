...
class AudioPlayerHelper(private val context: Context) { 
    ...
    var currentPlayingSentenceIndex by mutableStateOf<Int?>(null)
        private set
    ...
    fun playSentenceText(text: String, onComplete: (() -> Unit)? = null) { 
        speakTts(text, AudioTrackPlaying.SENTENCE, onComplete)
    }
    
    fun playStorySentences( 
        ...
        fun playNext() { 
            if (currentIndex >= sentences.size) { 
                currentPlayingSentenceIndex = null
                onSentenceChange(-1)
                onComplete()
                return
            }
            
            val sent = sentences[currentIndex]
            val thisIdx = currentIndex
            currentPlayingSentenceIndex = thisIdx
            onSentenceChange(thisIdx)
            
            speakTts(sent, AudioTrackPlaying.SENTENCE) { 
                mainHandler.postDelayed({ 
                    currentIndex++
                    playNext()
                }, 400L)
