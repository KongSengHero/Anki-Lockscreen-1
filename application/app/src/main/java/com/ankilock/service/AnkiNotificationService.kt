...
class AnkiNotificationService : Service() { 
    ...
    private fun setupMediaSession() { 
        mediaSession = MediaSessionCompat(this, "BlossomSession").apply { 
    ...
    private fun buildFlashcardNotification( 
        ...
        val kanjiRubyBitmapExpanded = if (isRevealed && kanjiFurigana.isNotBlank()) { 
            RubyTextRenderer.renderRubyBitmap( 
                context = this, 
        ...
        val kanjiRubyBitmapCollapsed = if (isRevealed && kanjiFurigana.isNotBlank()) { 
            RubyTextRenderer.renderRubyBitmap( 
                context = this, 
                rawText = vocabRubyText, 
                baseTextSizeSp = 20f, 
        ...
        val viewsAndRuby = listOf( 
            Triple(collapsedViews, rubyBitmapCollapsed, kanjiRubyBitmapCollapsed), 
            Triple(expandedViews, rubyBitmapExpanded, kanjiRubyBitmapExpanded) 
        ) 
        
        viewsAndRuby.forEach { (rv, rubyBitmap, kanjiRuby) -> 
            ...
            if (!isRevealed) { 
                ...
                rv.setViewVisibility(R.id.iv_kanji_ruby, View.GONE) 
                rv.setViewVisibility(R.id.tv_kanji, View.VISIBLE) 
                ...
                if (kanjiRuby != null) { 
                    rv.setImageViewBitmap(R.id.iv_kanji_ruby, kanjiRuby) 
                    rv.setViewVisibility(R.id.iv_kanji_ruby, View.VISIBLE) 
                    rv.setViewVisibility(R.id.tv_kanji, View.GONE) 
                    rv.setViewVisibility(R.id.tv_kanji_furigana, View.GONE) 
                } else { 
                    rv.setViewVisibility(R.id.iv_kanji_ruby, View.GONE) 
                    rv.setViewVisibility(R.id.tv_kanji, View.VISIBLE) 
                    ...
                    if (cleanReading.isNotBlank() && cleanReading != kanjiText) { 
                        rv.setTextViewText(R.id.tv_kanji_furigana, cleanReading) 
