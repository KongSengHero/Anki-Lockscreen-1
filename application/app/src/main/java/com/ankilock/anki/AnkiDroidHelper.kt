...
class AnkiDroidHelper(private val context: Context) { 
    ...
    fun getDeckStatsFromProvider(deckName: String): Triple<Int, Int, Int>? { 
        val cleanDeck = deckName.trim() 
        if (cleanDeck.isEmpty()) return null 
        val uris = listOf(DECKS_URI, Uri.parse("content://$AUTHORITY/decks")) 
        for (uri in uris) { 
            try { 
                val cursor = resolver.query(uri, null, null, null, null) 
                cursor?.use { cur -> 
                    ...
                    while (cur.moveToNext()) { 
                        val name = cur.getString(nameIdx) ?: continue 
                        if (name.equals(cleanDeck, ignoreCase = true)) { 
                            var newC = 0 
                            var learnC = 0 
                            var revC = 0 
                            if (countsIdx != null && countsIdx >= 0) { 
    ...
    fun getDueCardsForDeck( 
        ...
        try { 
            val selection = if (deckId > 0L) "deckID=$deckId, limit=${limit * 2}" else "limit=${limit * 2}" 
            ...
            cursor?.use { cur -> 
                while (cur.moveToNext() && result.size < limit) { 
                    val noteId = cur.getLong(cur.getColumnIndexOrThrow(COL_NOTE_ID)) 
                    if (seenNoteIds.contains(noteId)) continue 
                    if (excludeNoteId != null && noteId == excludeNoteId && cur.count > 1) continue 
                    seenNoteIds.add(noteId) 
                    
                    val cardOrd = cur.getInt(cur.getColumnIndexOrThrow(COL_CARD_ORD)) 
                    val buttonCount = cur.getInt(cur.getColumnIndexOrThrow(COL_BUTTON_COUNT)) 
                    val nextTimes = cur.getString(cur.getColumnIndexOrThrow(COL_NEXT_REVIEW_TIMES)) ?: "" 
    ...
    fun getNextDueCard(deckIds: Set<Long> = emptySet(), excludeNoteId: Long? = null): CardInfo? { 
        ...
        for (deckId in queryDeckIds) { 
            val targetDeckName = decks.find { it.id == deckId }?.name ?: "" 
            val cards = getDueCardsForDeck(deckId ?: -1L, limit = 2, excludeNoteId = excludeNoteId, deckName = targetDeckName) 
    ...
    fun getNextDueCard(deckId: Long?, excludeNoteId: Long? = null): CardInfo? { 
        if (deckId == null || deckId <= 0L) return getNextDueCard(emptySet(), excludeNoteId) 
        val targetDeckName = getDeckList().find { it.id == deckId }?.name ?: "" 
        val cards = getDueCardsForDeck(deckId, limit = 2, excludeNoteId = excludeNoteId, deckName = targetDeckName) 
        return cards.firstOrNull() 
    }
    
    fun getDistinctDueCards(deckIds: Set<Long> = emptySet(), limit: Int = 10): List<CardInfo> { 
        val decks = getDeckList()
        val queryDeckIds = if (deckIds.isNotEmpty()) deckIds.toList() else listOf<Long?>(null)
        val result = mutableListOf<CardInfo>()
        val seenNoteIds = mutableSetOf<Long>()
        ...
        for (deckId in queryDeckIds) { 
            try { 
                val selection = if (deckId != null) "deckID=$deckId, limit=${limit * 4}" else "limit=${limit * 4}"
                val cursor = resolver.query( 
                    SCHEDULE_URI, 
                    null, 
                    selection, 
    ...
    fun selectDeck(deckId: Long): Boolean { 
        if (deckId <= 0L) return false 
        return try { 
            val values = ContentValues().apply { 
                put(COL_DECK_ID, deckId) 
            } 
            resolver.update(SELECTED_DECK_URI, values, null, null) > 0 
        } catch (e: Exception) { 
            false 
        } 
    } 
